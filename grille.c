...
/**
 * fonction permettant d'afficher la grille vide et une légende dans la console.
 */
...
/**
 *fonction permettant à l'ordinateur de choisir la grille pour le joueur et l'IA (1 seule possibilité en version 0.1) et de créer le tableau associé
 */
void grilleChoisie(){
    //Sélection de la grille (fixe en v0.1
    int grille[NBLIGNE][NBCOLONNE] = GRID1;
    //création des tableaux utilisés pour le joueur et l'IA
    //variable pour la boucle for
    int i=0;
    int j=0;
    //boucle pour assigner les valeurs de la grille aux cases des tableau joueur et IA
    for (i=0;i<10;i++){
        for (j=0;j<10;j++){
            grilleJoueur[i][j]=grille[i][j];
